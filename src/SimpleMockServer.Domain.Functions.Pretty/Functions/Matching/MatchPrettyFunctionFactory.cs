...
namespace SimpleMockServer.Domain.Functions.Pretty.Functions.Matching;

internal class MatchPrettyFunctionFactory
{
    private readonly Dictionary<string, Type> _functionNameToType;
    private readonly IServiceProvider _serviceProvider;

    public MatchPrettyFunctionFactory(IServiceProvider serviceProvider)
    {
    ...
    internal bool TryCreate(string value, [MaybeNullWhen(false)] out IMatchPrettyFunction function)
    {
        function = null;

        (string functionName, string? argument) = value.SplitToTwoParts(":").Trim();

        if (!_functionNameToType.TryGetValue(functionName, out Type? functionType))
            return false;

        var prettyMatchFunction = (IMatchPrettyFunction)_serviceProvider.GetRequiredService(functionType);

        if (argument != null && prettyMatchFunction is not IWithArgumenFunction)
            throw new Exception($"Function '{functionName}' not support arguments");

        if (prettyMatchFunction is IWithArgumenFunction withArgument)
        {

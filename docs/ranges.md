...
- `interval` - интервал в котором будут находится значения
- `ranges` - названия диапазов внутри интервала 
- `capacity` - емкость одного диапазона
- `unit` - минимальная единица внутри интервала, требуется для настройки шага 
при разбиении на диапазоны. Настройка характерна только для типа `float`
...
Если значение не указано, то используется интервал `1 - 9223372036854775806`
...
Если значение не указано, то оно рассчитывается автоматическу путем деленение 
значения _интервал_ на количество диапазонов.
...
```
{
    "id": {
        "type": "int",
        "interval": "1 - 9_223_372_036_854_775_806",
        "mode": "seq",
        "ranges": [
          "first",
          "second",
...
***минимальная_единица*** - минимальная единица внутри интервала, требуется для настройки шага
при разбиении на диапазоны.
...
Если значение не указано, то используется интервал `<минимальная_единица> - 1_000_000`.
...
Если значение не указано, то оно рассчитывается автоматическу путем деленение
значения интервала `-9223372036854775808 - 9223372036854775806` на количество диапазонов.

Это значение требуется, если хочется получить значения и где-то их сохранить, 
т.к. если не указать _емкость_диапазона_, то величина будет изменяться от 
количества диапазонов и сохраненные значения могут стать не актуальными.
